...
class ToolChainJavaConfiguratorSpec {
    ...
    fun `When configure is called it setup up the ToolChain for the JavaExtension`() {
        ...
        // When
        ToolChainJavaConfigurator.configure(project)

        // Then
        verify(exactly = 1) { jvmToolchain.languageVersion.set(JavaLanguageVersion.of(BuildConfig.javaToolchainVersion)) }
    }

    @Test
    fun `When configure is called ignores all explicit Non Jvm ToolChains`() {
        // Given
    ...
    fun `When configure is called configures all plain Jvm ToolChains`() {
        ...
        verify(exactly = 1) {
            proof.languageVersion.set(JavaLanguageVersion.of(BuildConfig.javaToolchainVersion))
        }
    }

    @Test
    fun `When configure is called configures all KMP Jvm ToolChains`() {
        // Given
        val toolChain: KotlinJavaToolchain.JavaToolchainSetter = mockk(relaxed = true)
        val provider: Provider<JavaLauncher> = mockk()
        val jvmTask: KotlinCompile = mockk(relaxed = true) {
    ...
    fun `It fulfils ParameterlessConfigurator`() {
        val configurator = ToolChainJavaConfigurator as Any

        assertTrue { configurator is ParameterlessConfiguratorContract }
    }
}
