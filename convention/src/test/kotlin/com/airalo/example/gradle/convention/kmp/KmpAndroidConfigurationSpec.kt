...
class KmpAndroidConfigurationSpec {
    ...
    fun `Given apply is called it applies Kmp and the AndroidConvention Plugin`() {
        ...
        KmpAndroidConfiguration(
            androidLibraryConvention = androidLibraryConfiguration,
        ...
        verifyOrder {
            ...
            androidLibraryConfiguration.apply(project)
        }
    }

    @Test
    fun `When apply is called it configures Kmp for Android with iOS`() {
        // Given
        fakeProject.plugins.apply(BuildConfig.androidLibraryId)
        fakeProject.plugins.apply(BuildConfig.kmpId)
        ...
        // Then
        verify(exactly = 1) { kmpExtension.applyDefaultHierarchyTemplate() }
        verify(exactly = 1) { kmpExtension.androidTarget() }
    }
    ...
    fun `It fulfils Plugin`() {
        assertTrue {
            ...
            KmpAndroidConfiguration() is Plugin<*>
        }
    }
}
