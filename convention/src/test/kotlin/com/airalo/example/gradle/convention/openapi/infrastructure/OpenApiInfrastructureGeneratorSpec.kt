...
class OpenApiInfrastructureGeneratorSpec {
    ...
    fun `When apply is called it applies the KMP Android Configuration`() {
        ...
        // When
        OpenApiInfrastructureGenerator(kmpAndroidConfiguration).apply(project)
    ...
    @Test
    fun `When apply is called it registers and wires tasks to generate the Openapi infrastructure part`() {
        // Given
        every {
            extensionContainer.getByType(KotlinMultiplatformExtension::class.java)
        } returns mockk(relaxed = true) {
        ...
        // Then
        val generatorTask = project.tasks.findByName("openApiInfrastructureGenerate")
        assertTrue(generatorTask is OpenApiInfrastructureTask, "It does not contain the Infrastructure Generator Task")
        assertEquals(
    ...
    fun `When apply is called it sets a dependency to the openApiInfrastructureGenerate tasks for all compilations`() {
        ...
        // Then
        val infrastructureTask = project.tasks.findByName("openApiInfrastructureGenerate")
        ...
        assertTrue(
            actual = commonCompilation.dependsOn.toList().contains(infrastructureTask),
            message = "Expected commonCompilation to depend on infrastructureTask",
        )
        assertTrue(
            actual = commonCompilation.mustRunAfter.getDependencies(infrastructureTask).toList().contains(infrastructureTask),
            message = "Expected commonCompilation to run after infrastructureTask",
        )
        assertTrue(
            actual = jvmCompilation.dependsOn.toList().contains(infrastructureTask),
            message = "Expected jvmCompilation to depend on infrastructureTask",
        )
        assertTrue(
            actual = jvmCompilation.mustRunAfter.getDependencies(infrastructureTask).toList().contains(infrastructureTask),
            message = "Expected jvmCompilation to run after infrastructureTask",
        )
        assertTrue(
            actual = nativeCompilation.dependsOn.toList().contains(infrastructureTask),
            message = "Expected nativeCompilation to depend on infrastructureTask",
        )
        assertTrue(
            actual = nativeCompilation.mustRunAfter.getDependencies(infrastructureTask).toList().contains(infrastructureTask),
            message = "Expected nativeCompilation to run after infrastructureTask",
        )
    }
    ...
    fun `It fulfils Plugin`() {
        assertTrue {
            ...
            OpenApiInfrastructureGenerator(kmpAndroidConfiguration) is Plugin<*>
        }
    }
}
